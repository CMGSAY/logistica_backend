...
const crearAsignacionRuta = async (req, res) =>{
    try {
        ...
        const resultado = await Asignaciones_rutaModel.query(
            "INSERT INTO asignacion_ruta (envio_id, ruta_id, conductor_id, vehiculo_id, fecha_asignacion) VALUES ($1, $2, $3, $4, $5) RETURNING *",
            [envio_id, ruta_id, conductor_id, vehiculo_id, fecha_asignacion] 
        );
        res.status(201).json(resultado.row[0]); 
    }catch(error){
...
const actualizarAsignacionRuta = async (req, res) =>{
    try{
        ...
        const {envio_id, ruta_id, conductor_id, vehiculo_id, fecha_asignacion} = req.body;
        const resultado = await Asignaciones_rutaModel.query(
            "UPDATE asignacion_ruta SET envio_id = $1, ruta_id = $2, conductor_id = $3, vehiculo_id = $4, fecha_asignacion = $5 WHERE id = $6 RETURNING *",
...
const eliminarAsignacionRuta = async (req, res) =>{
    try{
        const {id} = req.params;
        const resultado = await Asignaciones_rutaModel.query("DELETE FROM asignacion_ruta WHERE id = $1 RETURNING *", [id]);
        if (resultado.rows.length === 0){
            return res.status(404).json({message:"Asignacion de ruta no encontrada"});
        }
