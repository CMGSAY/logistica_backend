...
const obtenerClientes = async (req, res) => {
    try {
        const resultado = await ClientesModel.query("SELECT * FROM clientes");
...
const obtenerClientesPorId = async (req, res) => {
    try {
        const { id } = req.params;
        const resultado = await ClientesModel.query("SELECT * FROM clientes WHERE id = $1", [id]);
...
const crearClientes = async (req, res) => {
    try {
        ...
        const resultado = await ClientesModel.query(
            `INSERT INTO clientes (${columns}) VALUES (${placeholders}) RETURNING *`,
            values
        );
...
const eliminarClientes = async (req, res) => {
    try {
        const { id } = req.params;
        const resultado = await ClientesModel.query("DELETE FROM clientes WHERE id = $1 RETURNING *", [id]);
